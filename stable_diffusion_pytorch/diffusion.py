...
class Conv2dSequential(nn.Sequential):
    def forward(self, x: torch.Tensor, context: torch.Tensor, time: torch.Tensor) -> torch.Tensor:
        for layer in self:
            x = layer(x)
        return x
...
class UNet(nn.Module):
    def __init__(self):
        ...
        self.encoders = nn.ModuleList([
            Conv2dSequential(nn.Conv2d(4, 320, kernel_size=3, padding=1)),
            SwitchSequential(ResidualBlock(320, 320), AttentionBlock(8, 40)),
            SwitchSequential(ResidualBlock(320, 320), AttentionBlock(8, 40)),
            Conv2dSequential(nn.Conv2d(320, 320, kernel_size=3, stride=2, padding=1)),
            SwitchSequential(ResidualBlock(320, 640), AttentionBlock(8, 80)),
            SwitchSequential(ResidualBlock(640, 640), AttentionBlock(8, 80)),
            Conv2dSequential(nn.Conv2d(640, 640, kernel_size=3, stride=2, padding=1)),
            ...
            Conv2dSequential(nn.Conv2d(1280, 1280, kernel_size=3, stride=2, padding=1)),
